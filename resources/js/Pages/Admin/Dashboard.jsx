...
// Fungsi Download PNG (Menggunakan html-to-image)
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button 
                ...
                className="relative w-10 h-10 flex items-center justify-center rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
            >
                <i className="fa-regular fa-bell text-lg"></i>
                {notifications.length > 0 && (
                    <span className="absolute top-2 right-2.5 w-2.5 h-2.5 bg-rose-500 border-2 border-white dark:border-slate-800 rounded-full"></span>
                )}
            </button>

            {isOpen && (
                <div className="absolute right-0 mt-3 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl z-50 overflow-hidden animation-fade-in-up origin-top-right">
                    ...
                    <div className="max-h-[300px] overflow-y-auto custom-scrollbar">
                        {notifications.length > 0 ? (
                            notifications.map((notif, idx) => (
                                <div key={idx} className="p-4 border-b border-slate-50 dark:border-slate-800/50 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <div className="p-3 border-t border-slate-100 dark:border-slate-800 text-center">
                        <button onClick={() => setIsOpen(false)} className="text-xs font-bold text-blue-600 hover:text-blue-700 dark:text-blue-400">
                            Tutup
                        </button>
                    </div>
                </div>
...
const StatCard = ({ id, title, value, subValue, icon, color = 'blue', trend }) => {
    const theme = CARD_THEMES[color] || CARD_THEMES.blue;

    return (
        <div id={id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 relative overflow-hidden group hover:border-slate-300 dark:hover:border-slate-700 transition-all shadow-sm hover:shadow-md">
            
            <DownloadButton targetId={id} fileName={title.replace(/\s+/g, '_')} />

            <div className={`absolute -right-6 -top-6 w-24 h-24 rounded-full ${theme.decor} opacity-5 dark:opacity-10 group-hover:scale-125 transition-transform duration-500 blur-2xl`}></div>
            
            <div className="relative z-10 flex justify-between items-start">
                <div className="pr-6">
                    <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-2">{title}</p>
                    <h3 className="text-3xl font-black text-slate-800 dark:text-white tracking-tight leading-none mb-3">{value}</h3>
                    
                    {subValue && (
                        <div className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-bold ${trend === 'up' ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-slate-50 text-slate-500'}`}>
                ...
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl shadow-sm ${theme.iconBg} ${theme.iconText}`}>
                    <i className={icon}></i>
                </div>
            </div>
        </div>
    ...
    return (
        <div className="relative h-80 w-full mt-4">
            <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full overflow-visible">
                ...
                <polyline fill="none" stroke="#3b82f6" strokeWidth="2" points={points} vectorEffect="non-scaling-stroke" strokeLinecap="round" />
                {chartData.map((d, i) => {
                    const x = (i / (chartData.length - 1)) * 100;
                    const y = 100 - ((d.count / maxVal) * 80);
                    return (
                        <circle key={i} cx={x} cy={y} r="3" className="fill-white stroke-blue-600 stroke-2 hover:r-5 transition-all cursor-pointer" />
                    );
                })}
            </svg>
...
const CareerStats = ({ id, stats, topPositions }) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                ...
                <div className="md:col-span-4 flex flex-col items-center justify-center">
                    <div className="relative w-28 h-28 rounded-full flex items-center justify-center shadow-inner"
                         style={{ 
                             background: `conic-gradient(#3b82f6 ${rate}%, #f1f5f9 ${rate}% 100%)` 
                         }}
                    >
                        <div className="absolute inset-0 rounded-full opacity-0 dark:opacity-100 pointer-events-none"
                             style={{ background: `conic-gradient(#3b82f6 ${rate}%, #1e293b ${rate}% 100%)` }}
                        ></div>
                    ...
                    <div className="flex gap-3 mt-4 w-full justify-center">
                        <div className="text-center">
                            <p className="text-sm font-bold text-slate-800 dark:text-white">{stats.employed}</p>
                        ...
                        <div className="text-center">
                            <p className="text-sm font-bold text-slate-400">{stats.unemployed}</p>
                    ...
                    {topPositions.length > 0 ? topPositions.map((pos, idx) => (
                        <div key={idx} className="group/item">
                            ...
                            <div className="h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                                <div 
                                    className="h-full bg-emerald-500 rounded-full group-hover/item:bg-emerald-400 transition-colors"
                                    style={{ width: `${(pos.total / maxPosValue) * 100}%` }}
                                ></div>
                            </div>
                        </div>
                    )) : (
    ...
    return (
        <AdminLayout>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
                <StatCard id="stat-alumni" title="TOTAL ALUMNI" value={stats.alumni.total} subValue={stats.alumni.new > 0 ? `+${stats.alumni.new} Baru` : null} trend="up" icon="fa-solid fa-user-graduate" color="blue" />
                <StatCard id="stat-jobs" title="LOKER AKTIF" value={stats.jobs.total} subValue={stats.jobs.new > 0 ? `+${stats.jobs.new} Bulan Ini` : null} trend="up" icon="fa-solid fa-briefcase" color="purple" />
                <StatCard id="stat-events" title="AGENDA KEGIATAN" value={stats.events} icon="fa-solid fa-calendar-check" color="teal" />
                <StatCard id="stat-pending" title="USER PENDING" value={stats.pending} subValue={stats.pending > 0 ? "Butuh Verifikasi" : "Aman"} icon="fa-solid fa-user-clock" color="rose" />
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-2 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div id="chart-graduation" className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm relative group overflow-visible">
                            ...
                            <div className="h-80 flex items-end gap-3">
                                {graduationData.length > 0 ? graduationData.map((data, index) => (
                                    <div key={index} className="flex-1 flex flex-col items-center gap-2 group cursor-pointer h-full justify-end relative hover:z-20">
                                        
                                        {/* BAR VISUAL */}
                                        <div 
                                            className="w-full bg-indigo-200 dark:bg-indigo-600 hover:bg-indigo-500 dark:hover:bg-indigo-500 rounded-t-md transition-all relative"
                                            style={{ height: `${(data.total / maxGradValue) * 80}%` }}
                                        >
                                            {/* TOOLTIP: MOVED INSIDE THE BAR FOR RELATIVE POSITIONING */}
                                    ...
                                    <div className="w-full h-full flex items-center justify-center text-slate-400 text-xs italic border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-lg">
                                        Belum Ada Data
                                    </div>
                                )}
                            </div>
                        ...
                        <div id="chart-growth" className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm relative group">
                            ...
                            <div className="flex justify-between items-start mb-2 pr-8">
                                ...
                                <div className="text-emerald-600 dark:text-emerald-400 text-[10px] font-bold bg-emerald-100 dark:bg-emerald-400/10 px-2 py-1 rounded flex items-center gap-1">
                    ...
                    <CareerStats id="chart-career" stats={careerStats} topPositions={topPositions} />
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-5">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <Link href={route('admin.news.create')} className="bg-white dark:bg-slate-900 hover:bg-amber-50 dark:hover:bg-amber-900/20 border border-slate-200 dark:border-slate-700 hover:border-amber-300 dark:hover:border-amber-500/50 p-3 rounded-lg flex flex-col items-center justify-center gap-2 transition-all group shadow-sm h-24">
                                <i className="fa-solid fa-pen-nib text-amber-500 group-hover:scale-110 transition-transform text-lg"></i>
                            ...
                            <Link href={route('admin.jobs.create')} className="bg-white dark:bg-slate-900 hover:bg-purple-50 dark:hover:bg-purple-900/20 border border-slate-200 dark:border-slate-700 hover:border-purple-300 dark:hover:border-purple-500/50 p-3 rounded-lg flex flex-col items-center justify-center gap-2 transition-all group shadow-sm h-24">
                                <i className="fa-solid fa-briefcase text-purple-500 group-hover:scale-110 transition-transform text-lg"></i>
                                <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 group-hover:text-purple-600 dark:group-hover:text-purple-300">Tambah Loker</span>
                            </Link>
                            <Link href={route('admin.alumni.create')} className="bg-white dark:bg-slate-900 hover:bg-blue-50 dark:hover:bg-blue-900/20 border border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-500/50 p-3 rounded-lg flex flex-col items-center justify-center gap-2 transition-all group shadow-sm h-24">
                            ...
                            <Link href={route('admin.alumni.export')} className="bg-white dark:bg-slate-900 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 border border-slate-200 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-500/50 p-3 rounded-lg flex flex-col items-center justify-center gap-2 transition-all group shadow-sm h-24">
                                <i className="fa-solid fa-file-export text-emerald-500 group-hover:scale-110 transition-transform text-lg"></i>
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <span className="text-[10px] text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded font-mono">
                                Real-time
                            </span>
                        </div>
                        ...
                        <div className="relative space-y-6 pl-2">
                            ...
                            {activityLog.map((log, idx) => (
                                <div key={idx} className="relative pl-6 group">
                                    <div className={`absolute left-0 top-1 w-6 h-6 rounded-full border-4 border-white dark:border-slate-900 z-10 flex items-center justify-center text-[8px] text-white font-bold shadow-sm ${log.color.replace('text-', 'bg-')}`}>
                                        <i className={`fa-solid ${log.icon}`}></i>
                                    </div>
                                    <div>
                                        <p className="text-[11px] text-slate-600 dark:text-slate-300 font-medium leading-tight group-hover:text-slate-900 dark:group-hover:text-white transition-colors">
                                            {log.message}
                                        </p>

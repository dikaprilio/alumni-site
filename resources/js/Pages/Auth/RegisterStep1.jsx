            ...
            <div className="relative z-10 w-full max-w-md px-6">
                <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-white/20 dark:border-slate-700 shadow-2xl rounded-3xl p-8 animate-fade-in-up">
                    ...
                    <div className="text-center mb-6">
                        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Verifikasi Alumni</h2>
                        <p className="text-slate-500 dark:text-slate-400 text-sm mt-2">
                            Langkah 1: Masukkan NIM Anda untuk pengecekan data.
                        </p>
                    </div>

                    {/* Progress Bar */}
                    <div className="w-full bg-slate-200 dark:bg-slate-700 h-1.5 rounded-full mb-6 overflow-hidden">
                        <div className="bg-brand-600 h-1.5 rounded-full w-1/2 transition-all duration-500"></div>
                    </div>

                    <form onSubmit={submit} className="space-y-5">
                        <div>
                            <label className="block text-xs font-bold uppercase text-slate-500 dark:text-slate-400 mb-2 tracking-wider">Nomor Induk Mahasiswa (NIM)</label>
                            <input
                                type="text"
                                value={data.nim}
                                onChange={(e) => setData('nim', e.target.value)}
                                className="w-full px-4 py-3 rounded-xl bg-white/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none transition-all dark:text-white font-mono tracking-wider"
                                placeholder="J3xxxxxx"
                            />
                        ...
                        >
                            {processing ? 'Mengecek...' : 'Lanjut Langkah 2'}
                        </button>
                    </form>

                    <div className="mt-8 text-center">
                        <Link href="/login" className="text-slate-500 hover:text-brand-600 text-sm font-medium transition-colors">
